...
export function ChatView({
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg-app)]">
      ...
      {availableQuestions.length > 0 && messages.length > 0 && !isSending && analysis && (
        <div className="flex-shrink-0 px-4 py-2 border-t border-[var(--border-subtle)] bg-[var(--bg-app)]">
          <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
            {availableQuestions.map((question, index) => (
              <button
                ...
                onClick={() => handleSuggestedQuestion(question)}
                disabled={isSending || isLoading}
                className="flex-shrink-0 text-xs font-mono px-3 py-1.5 rounded-full transition-all duration-150 bg-[var(--bg-card)] border border-[var(--border-subtle)] hover:border-[var(--accent-primary)] hover:text-[var(--accent-primary)] disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {question}
              </button>
            ))}
