import { callOpenRouterWithVision, callOpenRouter } from './openrouter.js';
...
export async function categorizeColorsWithVision(
  ...
  const response = await callOpenRouterWithVision(
    ...
    prompt,
    screenshot,
    { temperature: 0.2, maxTokens: 800, apiKey: options?.apiKey, model: options?.model }
  );
...
export async function categorizeColorsTextOnly(
  ...
  ], { temperature: 0.2, maxTokens: 500, apiKey: options?.apiKey, model: options?.model });
